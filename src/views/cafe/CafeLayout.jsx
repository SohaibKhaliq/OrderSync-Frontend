    ...
    <div className="min-h-screen bg-theme-light text-theme-dark font-sans flex flex-col">
      ...
      <nav className="navbar bg-transparent px-4 py-6 md:px-12 xl:px-24 flex items-center justify-between border-b border-base-200">
        <div className="flex-1 lg:flex-none">
          <Link to="/" className="text-3xl font-serif font-bold tracking-tight text-secondary">
        ...
        <div className="flex-none flex items-center gap-4">
          ...
          {customer ? (
            <div className="dropdown dropdown-end">
              <div tabIndex={0} role="button" className="btn btn-ghost !px-2 hidden md:inline-flex">
            ...
            <div className="flex items-center gap-2">
              <Link to="/login" className="font-semibold text-sm hover:text-primary transition-colors hidden md:block">Log In</Link>
              <Link to="/register" className="btn btn-primary rounded-full px-6 font-semibold">Sign Up</Link>
            </div>
          )}
        </div>
      </nav>
      ...
      <footer className="bg-[#111111] text-white pt-20 pb-8 px-6 md:px-12 xl:px-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          {/* Col 1 */}
          <div>
            <Link to="/" className="text-3xl font-serif font-bold tracking-tight text-white mb-6 inline-block">
              {store?.store_name || "DINING"}
              <span className="text-primary text-xl relative -top-2">®</span>
            </Link>
            <p className="text-sm opacity-70 leading-relaxed max-w-sm">
              For us, it's not just about making a great impression, it's about raising the bar and having everything handled perfectly.
            </p>
          </div>
          
          {/* Col 2 */}
          <div>
            <h4 className="text-xs font-bold tracking-wider mb-6 text-gray-400">OPEN HOURS</h4>
          ...
          {/* Col 3 */}
          <div>
            <h4 className="text-xs font-bold tracking-wider mb-6 text-gray-400">MORE INFO</h4>
            <ul className="text-sm space-y-4 opacity-80">
              ...
              <li><Link to="/contact" className="hover:text-primary">Contact Now</Link></li>
            </ul>
          </div>

          {/* Col 4 */}
          <div>
            <h4 className="text-xs font-bold tracking-wider mb-6 text-gray-400">ADDRESS</h4>
            <div className="text-sm space-y-4 opacity-80 mb-6">
              <p>{store?.address || "329 Queensberry Street,\nNorth Melbourne VIC 3051, Australia."}</p>
            </div>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs opacity-50">
          <p>© {new Date().getFullYear()} {store?.store_name || "OrderSync"}. All Rights Reserved.</p>
